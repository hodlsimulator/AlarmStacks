...
struct LiveActivityTheme {
    ...
    // Accent colours should match your in-app theme chips.
    private static func accentColor(for name: String) -> Color {
        ...
        default:         return Color(red: 0.04, green: 0.52, blue: 1.00) // Default iOS blue
        }
    }

    // Pastel/complementary background for the Live Activity card.
    // Kept subtle to maintain text contrast on all surfaces.
    private static func backgroundColor(for name: String, scheme: ColorScheme) -> Color {
        switch (name, scheme) {

        // Default (blue → warm peach/amber)
